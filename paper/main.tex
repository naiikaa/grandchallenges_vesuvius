...
Recent work on ink detection for the Herculaneum papyri has been driven largely by the Vesuvius Challenge and closely related research since 2023. The EduceLab‑Scrolls project formalizes the ink‑detection task by releasing a large multimodal dataset (CT volumes, meshes, IR images, and aligned ink masks) together with a CT‑to‑text pipeline in which a learned model predicts ink on “surface volumes” and is validated both quantitatively and by expert review. This work defines the modern benchmark setting for ink detection and demonstrates that models trained on fragment data can generalize to “hidden layers” in rolled scrolls. \cite{parsons2024educelabscrollsverifiablerecoverytext} The Vesuvius Challenge 2023 “Ink Detection” Kaggle competition reframes this as a 2D segmentation problem on three annotated fragments and catalyzes a range of deep learning architectures. The winning solution combines 3D convolutional backbones (3D U‑Nets and related models) to extract volumetric features from 65‑slice surface volumes with a 2D transformer‑based SegFormer head operating on depth‑flattened features, achieving the top F0.5 score on the public leaderboard and outperforming purely 2D UNet‑style baselines. Other high‑ranking entries explore 2D models over stacked slices, hybrid 2D/3D setups with PVT‑based decoders, strong tiling and augmentation, and show that explicitly handling depth variability is crucial for robust performance. \cite{kaggle_1st_place} \cite{kaggle_challenge_vesuvius}
Follow‑up prizes in 2023 focus on transferring these fragment‑trained models to full scrolls, highlighting challenges of resolution mismatch, noise, and papyrus morphology. Here, several groups introduce tools for labeling “crackle” patterns associated with ink, explore the effect of resolution on model behavior, and propose using fragment‑trained models to generate pseudo‑labels in scroll volumes, which are then used to train fully volumetric detectors. An important outcome is a 3D UNet‑based approach that back‑projects 2D ink predictions into the volume and learns a direct 3D ink segmentation model aimed at scaling to arbitrary regions and scrolls. 
The most recent advancement made in the task of ink detection was performed by a pair of participants winning the "First Title Price" in 2025. They utilized a UNETR (Vision Transformer for Medical Segmentation) to yield the presented results. They could reconstruct the title of the first title in the Herculaneum Scroll.
Recent work on ink detection for the Herculaneum papyri has been driven largely by the Vesuvius Challenge and closely related research since 2023. The EduceLab‑Scrolls project formalizes the ink‑detection task by releasing a large multimodal dataset (CT volumes, meshes, IR images, and aligned ink masks) together with a CT‑to‑text pipeline in which a learned model predicts ink on “surface volumes” and is validated both quantitatively and by expert review. This work defines the modern benchmark setting for ink detection and demonstrates that models trained on fragment data can generalize to “hidden layers” in rolled scrolls. The Vesuvius Challenge 2023 “Ink Detection” Kaggle competition reframes this as a 2D segmentation problem on three annotated fragments and catalyzes a range of deep learning architectures. The winning solution combines 3D convolutional backbones (3D U‑Nets and related models) to extract volumetric features from 65‑slice surface volumes with a 2D transformer‑based SegFormer head operating on depth‑flattened features, achieving the top F0.5 score on the public leaderboard and outperforming purely 2D UNet‑style baselines. Other high‑ranking entries explore 2D models over stacked slices, hybrid 2D/3D setups with PVT‑based decoders, strong tiling and augmentation, and show that explicitly handling depth variability is crucial for robust performance. Follow‑up prizes in 2023 focus on transferring these fragment‑trained models to full scrolls, highlighting challenges of resolution mismatch, noise, and papyrus morphology. Here, several groups introduce tools for labeling “crackle” patterns associated with ink, explore the effect of resolution on model behavior, and propose using fragment‑trained models to generate pseudo‑labels in scroll volumes, which are then used to train fully volumetric detectors. An important outcome is a 3D UNet‑based approach that back‑projects 2D ink predictions into the volume and learns a direct 3D ink segmentation model aimed at scaling to arbitrary regions and scrolls. The 2023 Vesuvius Grand Prize solution pushes model design further by importing architectures from video understanding. The winning team employs an ensemble of TimeSformer Small with divided space–time attention, I3D with non‑local blocks, and ResNet3D‑101, trained on small 3D patches around the papyrus surface to predict coarse ink probability grids on scroll data. Their training regime combines extensive iterative label refinement, targeted patch sampling around ink strokes, and strong regularization to reduce hallucinated letter completions, with performance assessed primarily through the amount and quality of continuous legible text recovered on intact scrolls. The subsequent release of the corresponding code in the official monorepo and extended forks establishes these 3D attention‑based models as the current de facto baseline for scroll‑level ink detection. In parallel, Quattrini et al. introduce Volumetric Fast Fourier Convolution (VFFC), a specialized 3D convolutional operator designed for volumetric ink detection and evaluated on Herculaneum fragment data. Integrated into a segmentation network, VFFC leverages FFT‑based convolutions to enlarge the receptive field at manageable cost and is reported to outperform standard 3D CNN baselines on ink‑detection benchmarks, with an open‑source implementation exposing patch‑based training and inference for surface volumes. \cite{first_title_prize_blog} \cite{first_title_prize_repo}
